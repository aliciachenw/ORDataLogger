...
class VoxelArray3D(object):
    ...
    def fillGaps(self, contVA, internalVA, method='VNN', blocksN=1, blockDir='X', distTh=None, maxS=3, minPct=0.):
        ...
        sliceMethod = 'slow'  # 'fast' is not working
        for b in range(0, blocksN):
            ...
            if sliceMethod == 'slow':
                ...
                idxBlock = np.zeros(int(np.prod(self.getDims())), dtype=bool)
                idxBlock[ind] = True
            if method == 'VNN':
                # Apply VNN
                if sliceMethod == 'slow':
...
class VoxelArray3DFrom2DImages(VoxelArray3D):
    ...
    def __init__(self, **kwargs):
        ...
        # Create voxel array for grey values indicating how many times a voxel
        # has been written
        ...
        self.internalV = VoxelArray3D(dataType=bool, dims=self.getDims())
        ...
        if wrapper == 'parallelepipedon':
            ...
            if not np.array_equal(cCurrent,cPrev):
                ...
                try:
                    ...
                    self.internalV.setDataByIdx(idxInternal, True) # here mark the internal points that needs to be filled
                except:
...
def getCoordsInConvexHull(p):
    ...
    xCube, yCube, zCube = getCubeCoords(([xMin,xMax],[yMin,yMax],[zMin,zMax])) # about 2s
    ci = np.array((xCube, yCube, zCube)).T
    # Linear interpolation
    vi = griddata(p, np.ones((p.shape[0],)), ci, method='linear', fill_value=0) # about 8s -> data dependent though
...
def nparray2vtkImageData(v, d, s, o, vtkScalarType):
    ...
    o : list
        3-elem list of origin of the ``vtk.vtkImageData`` (see `here <http://www.vtk.org/doc/nightly/html/classvtkImageData.html#ab3288d13810266e0b30ba0632f7b5b0b>`_).
    vtkScalarType :
    ...
    # Create source
    data = np.reshape(v, (int(d[0]), int(d[1]), int(d[2])), order='F')
    source = sitk.GetImageFromArray(data)
    source.SetSpacing((s[0], s[1], s[2]))
    source.SetOrigin((o[0], o[1], o[2]))
    return source
